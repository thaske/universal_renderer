...
RSpec.describe UniversalRenderer::StaticClient do
  ...
  describe '.static' do
    context 'when ssr_url is not configured' do
      before do
        UniversalRenderer.configure { |config| config.ssr_url = nil }
      end
    ...
    context 'when ssr_url is configured' do
      ...
      context 'and the SSR server responds successfully' do
        before do
          stub_request(:post, "#{ssr_url}/")
            .with(body: expected_request_body, headers: request_headers)
            .to_return(status: 200, body: expected_ssr_response_body.to_json, headers: { 'Content-Type' => 'application/json' })
        end

        it 'makes a POST request to the SSR server' do
          described_class.static(page_url, props)
          expect(WebMock).to have_requested(:post, "#{ssr_url}/")
            .with(body: expected_request_body, headers: request_headers).once
        end
      ...
      context 'when the SSR server responds with a non-OK status' do
        ...
        it 'logs an error' do
          expect(Rails.logger).to receive(:error).with(/SSR static request to .* failed: \d{3} - .*/)
          described_class.static(page_url, props)
        end
      end
      ...
      context 'when the request times out' do
        ...
        it 'logs an error' do
          ...
          expect(Rails.logger).to receive(:error).with(/SSR static request to .* failed: (Net::OpenTimeout|Net::ReadTimeout).*/)
          described_class.static(page_url, props)
        end
      end
      ...
      context 'when the SSR server responds with malformed JSON' do
        ...
        it 'logs an error' do
          expect(Rails.logger).to receive(:error).with(/SSR static request to .* failed: JSON::ParserError/)
          described_class.static(page_url, props)
        end
      end
    end
