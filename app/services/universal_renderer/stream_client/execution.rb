module UniversalRenderer
  class StreamClient
    module Execution
      class << self
        def _perform_streaming(http_client, http_post_request, response, stream_uri)
          http_client.request(http_post_request) do |node_res|
            response.headers['Content-Type'] = 'text/html; charset=utf-8'
            response.headers.delete('Content-Length')
            response.headers['X-Accel-Buffering'] = 'no'

            Execution._handle_node_response_streaming(node_res, response, stream_uri)
            return true
          end
          false
        end
        ...
        def _handle_node_response_streaming(node_res, response, stream_uri)
          if node_res.is_a?(Net::HTTPSuccess)
            ...
            Execution._handle_streaming_for_node_error(node_res, response, stream_uri)
          end
        ...
        def _stream_response_body(source_http_response, target_io_stream)
          source_http_response.read_body { |chunk| target_io_stream.write(chunk) }
        end
