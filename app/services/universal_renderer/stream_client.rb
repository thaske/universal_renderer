...
module UniversalRenderer
  class StreamClient
    ...
    def self.stream(url, props, response)
      config = UniversalRenderer.config
      unless Setup._ensure_ssr_server_url_configured?(config)
        Rails.logger.warn('StreamClient: SSR URL (config.ssr_url) is not configured. Falling back.')
        return false
      end
      ...
      begin
        actual_stream_uri, http_client, http_post_request =
          Setup._build_stream_request_components(url, props, config)
      ...
      Execution._perform_streaming(http_client, http_post_request, response, stream_uri_obj)
    rescue Errno::ECONNREFUSED,
           Errno::EHOSTUNREACH,
           Net::OpenTimeout,
           Net::ReadTimeout,
           SocketError => e
      uri_str_for_conn_error = stream_uri_obj ? stream_uri_obj.to_s : full_ssr_url_for_log
      ErrorLogger._log_connection_error(e, uri_str_for_conn_error)
      false
    rescue StandardError => e
      uri_str_for_unexpected_error = stream_uri_obj ? stream_uri_obj.to_s : full_ssr_url_for_log
      ErrorLogger._log_unexpected_error(e, uri_str_for_unexpected_error,
