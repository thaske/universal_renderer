...
const setup = async ({ req }: SetupParams) => {
  ...
  const name = req.body?.props?.name || "World"; // Assuming props are passed in req.body.props
  ...
  const state = { user: null, initialData: {}, nameFromSetup: name }; // Example initial state
