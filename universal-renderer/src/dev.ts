 ...
 *
 * A bare `import("vite")` resolves relative to *this* module, landing on the
 * package's own copy whenever it is linked. Two Vite instances disagree about
 * plugin state, and the symptom is not an import error but path aliases failing
...
export type DevServerOptions = {
   ...
   * client `vite.config.*` — the render needs the same plugins and aliases the
   * client build uses. Do NOT point this at the SSR build config, which omits
   * the Rails plugin.
   */
  ...
  /**
   * Transport options merged over the loaded config. Narrower than
   * `Partial<SsrConfig>` because the lifecycle hooks and `streamCallbacks` are
   * re-resolved per render, so overriding those would do nothing.
   *
 ...
 * transform, so edits apply without a rebuild.
 *
 * Development only. Production runs the prebuilt bundle (`defineSsrConfig` +
 * `startServer`), because a Vite dev server transforming modules per render is
 * the single largest cost in the SSR path.
 *
 * The config module is re-resolved on every render rather than pinned at boot,
 * so an edit is picked up in full. Pinning it would leave parts of the graph at
 * their old version, and a stale module-level singleton renders a page that
 * disagrees with the one the browser hydrates. All four hooks for a render come
 * from the same reloaded module.
 *
 * @example
 * ```ts
