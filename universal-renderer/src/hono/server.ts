...
/**
 * Creates a Hono server configured for Server-Side Rendering (SSR).
 *
 ...
 * @returns Promise that resolves to a configured Hono application
 *
 * @example
 * ```typescript
 ...
 * import { createHealthHandler, createSSRHandler } from 'universal-renderer/hono';
 *
 * const app = new Hono();
...
>(options: HonoServerOptions<TContext>): Promise<Hono> {
  if (!options.render) {
