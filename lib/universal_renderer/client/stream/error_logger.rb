module UniversalRenderer
  module Client
    class Stream
      module ErrorLogger
        class << self
          def log_setup_error(error, target_uri_string)
          ...
          def log_connection_error(error, target_uri_string)
            Rails.logger.error(
              "SSR stream connection to #{target_uri_string} failed: #{error.class.name} - #{error.message}"
            )
          end

          def log_unexpected_error(error, target_uri_string, context_message)
            backtrace_info = error.backtrace&.first || "No backtrace available"
            Rails.logger.error(
