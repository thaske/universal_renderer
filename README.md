...
- The request includes the current URL and any props added via `add_prop`.
...
- **Request to SSR Server**:
  ...
  - The JSON payload of this request includes:
    ...
    - `props`: Data passed from your Rails controller using `add_prop`.
...
### Passing Data to SSR (`add_prop`)

In your controllers, you can use the `add_prop` method to pass data from your Rails application to the SSR service. This data will be available as a JSON object under the `props` key in the JSON payload sent to your SSR service.

`add_prop` can be called in two ways:
...
1.  **Key-Value Pair:**
    ...
    def show
      ...
      add_prop(:product, @product.as_json) # Ensure data is serializable
      add_prop(:current_user_name, current_user.name)
...
2.  **Hash Argument:**
    ...
    def index
      @posts = Post.recent
      add_prop(posts: @posts.map(&:as_json), current_page: params[:page])
      # ... default_render will be called implicitly
    end
    ```
...
1.  **Static Rendering Endpoint (for `StaticClient`):**
    ...
    - **Request Body (JSON):**
      ```jsonc
      {
        ...
        "props": {
          // JSON object built from add_prop calls
...
2.  **Streaming Rendering Endpoint (for `StreamClient`):**
    ...
    - **Request Body (JSON):**
      ```jsonc
      {
        "url": "current_rails_request_original_url",
        "props": {
          // User-defined props from add_prop calls,
          // e.g., "product": { ... }, "current_user_name": "..."
        },
...
class ProductsController < ApplicationController
  def show
    ...
    # Pass data to SSR service using add_prop
    add_prop(
